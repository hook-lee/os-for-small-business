...
0. ⭐ **범용 SaaS — 특정 센터 전용 설계 금지.** Onmove는 **모든 운동센터 사장님용**이다. 라파는 첫 계정일 뿐 설계 기준이 아니다(2026-06-11 사용자 명시: "라파를 아예 잊어도 돼. 어떤 원장이 쓰더라도 커스텀 가능하게"). **모든 규칙·값·종류·문구(시급 종류·결제수단·카테고리·차감/급여 정책·알림 임계치·예약 룰 등)는 원장이 커스텀 가능해야 한다.** 코드에 고정 상수·한국어 키워드 매칭·특정 센터 가정을 박지 말 것. 새 값 만들 때마다 "센터마다 다를까?" 자문 → 다르면 처음부터 설정으로.
...
- **2025-07 ~ 2026-06**: 일반과세자 (12개월, 매출 1억400만 초과 → 강제 전환)
- **2026-07 ~**: 직전 1년 매출 < 1억400만 시 간이 전환 가능
