 ...
 * - ownerId는 buildTools(ownerId) factory를 통해 클로저로 주입 → tool 호출 인자로 노출되지 않음
 */
...
/**
 * ownerId를 클로저로 주입한 tool 배열을 만든다.
 * /api/chat에서 요청별 ownerId로 호출.
 */
export function buildTools(ownerId: string): ToolHandler[] {
  // ─────────────────────────────────────────────────────────
  // Tool 1: 회원 검색 (이름)
  // ─────────────────────────────────────────────────────────
  const searchMembers: ToolHandler = {
    ...
    execute: async (args) => {
      ...
      const all = await fetchAllMembers(ownerId)
  ...
  const getMemberDetail: ToolHandler = {
    ...
    execute: async (args) => {
      ...
      const member = await fetchMemberById(id, ownerId)
  ...
  const listInstructors: ToolHandler = {
    ...
    execute: async () => {
      const all = await fetchAllInstructors(ownerId)
      return all.map(i => ({
        id: i.id,
        name: i.name,
        role: i.role,
